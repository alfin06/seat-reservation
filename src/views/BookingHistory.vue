...
export default {
  ...
  computed: {
    uniqueRooms() {
      // Now ensuring that only room labels like "A", "B", "C" appear
      return ['A', 'B', 'C']; // Hardcoding room labels
    },
    dateFilterDisplay() {
    ...
    async fetchBookingHistory() {
      ...
      try {
        ...
        this.bookings = data.map(entry => ({
          id: entry.id,
          room: entry.classroom,  // Ensure 'classroom' exists in the response
          seat: entry.seat_name,  // Ensure 'seat_name' exists in the response
...
<style scoped>
/* Styling the calendar to make sure it is displayed correctly */
.v-calendar {
...
.v-date-picker {
  background-color: white !important;  /* Set the background of the date picker */
  color: black !important;  /* Ensure the date text is black */
}

.v-calendar .v-btn {
  background-color: #fff !important; /* Prevent dark background on buttons inside the calendar */
  color: black !important; /* Ensure the button text is visible */
}

.v-calendar .v-btn:hover {
  background-color: #f0f0f0 !important; /* Highlight effect for buttons */
}
